...
ADDITIONAL   := .config/gh/config.yml .config/ghostty/config .config/nvim/init.lua .config/nvim/lua/plugins.lua
...
deploy: ## Create symlinks to $HOME
	@mkdir -p $(HOME)/.config/gh $(HOME)/.config/ghostty $(HOME)/.config/nvim/lua $(HOME)/.claude
