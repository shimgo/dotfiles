...
-- nvim-ufo {{{
vim.o.foldcolumn = '1' -- '0' だと折り畳みの列が表示されない
...
require('ufo').setup({
  provider_selector = function(bufnr, filetype, buftype)
    return { 'treesitter', 'indent' } -- Treesitter優先、なければインデント
  end,
  -- 折り畳み表示をカスタマイズするハンドラ
...
vim.keymap.set('n', 'zM', function()
  ...
  vim.o.foldlevel = 0  -- foldlevelを0にリセット
end, { desc = "Close all folds" })

-- zR: 全ての折り畳みを開く
vim.keymap.set('n', 'zR', function()
  require('ufo').openAllFolds()
  vim.o.foldlevel = 99  -- foldlevelを最大値に
...
vim.keymap.set('n', 'zr', function()
  ...
  if current_level < 99 then
    vim.o.foldlevel = current_level + 1
  end
end, { desc = "Decrease fold level (fold less)" })
...
vim.keymap.set('n', 'zm', function()
  ...
  if current_level > 0 then
    if current_level == 99 then
      -- zR直後または初期状態の場合、最大折り畳みレベルを取得して調整
      local max_fold_level = 0
      for lnum = 1, vim.fn.line('$') do
        max_fold_level = math.max(max_fold_level, vim.fn.foldlevel(lnum))
      end
      if max_fold_level > 0 then
        vim.o.foldlevel = max_fold_level - 1
      else
        vim.o.foldlevel = 0  -- 折り畳みがない場合は0に
      end
    else
      vim.o.foldlevel = current_level - 1
    end
